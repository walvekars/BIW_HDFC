...
class PermanentRecords(models.Model):
    ...
    customer_name = fields.Many2one('res.partner', string="CLIENT, CUSTOMER", ondelete='restrict', store=True)
    ...
    order_no = fields.Many2one('sale.order', string='ORDER NO', ondelete='restrict')
    delivery_id = fields.Many2one('stock.picking', string='DELIVERY ID', ondelete='restrict')
    wip_date = fields.Date(string='WIP DATE', help='This date updates based on First WIP Boolean field, if it is false date updates here and sets tpo True')
    first_wip = fields.Boolean(string='First WIP', default=False, help="Called First wip because, There are many wip states Those are ODOO - Waiting and Confirmed, and BIW - Ready and Not Serviceable")
    hand_off_id = fields.Char(string='HAND-OFF ID', related='delivery_id.hand_off_id', store=True)
    hand_off_date = fields.Date(string='HAND-OFF Date')
    awb_nos = fields.Many2one('air.way.bill', string='AWB NO', related='delivery_id.awb_number', store=True)
    dispatched_on = fields.Date(string='DISPATCHED ON')
    ...
    up_pod_date = fields.Date(string='DELIVERY UPDATE')
    person_delv = fields.Char(string='PERSON DELV')
    return_date = fields.Date(string='RETURN DATE')
    up_return_date = fields.Date(string='RETURN UPDATE')
    ...
    re_dispatch_update = fields.Datetime(string='RE-DISPATCH UPDATE')
